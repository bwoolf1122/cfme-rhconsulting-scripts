...
#puts "Filename: #{filename}"
  ...
  def import_resource_actions(custom_button, resource_actions)
    ...
    #   end
    # end
    resource_action = ResourceAction.in_region(MiqRegion.my_region_number).find_or_create_by(resource_id: custom_button.id, resource_type: 'CustomButton')
    ra = {}
    ...
    custom_buttons.each do |b|
      ...
      custom_buttons.collect do |custom_button|
        button = custom_button.attributes.slice(
            ...
            'wait_for_complete', 'name', 'visibility', 'visibility_expression', 'enablement_expression', 'disabled_text', 'applies_to_id')
